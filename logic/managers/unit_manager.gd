...
func _perform_load_unit(unit_object_uids_array : Array[String]) -> void:
	if is_instance_valid(_current_units_array):
		for unit in _current_units_array:
			unit.queue_free()
		_current_units_array = []
	...
	for unit_object_uid in unit_object_uids_array:
		...
		var new_unit : Node = new_unit_packed.instantiate()
		...
		_current_units_array.append(new_unit as Node2D)
	
		if not is_instance_valid(unit_root):
			push_error(name + ": Unit Root not assigned!")
			return
		
		unit_root.add_child(_current_units_array[new_unit])
#endregion
